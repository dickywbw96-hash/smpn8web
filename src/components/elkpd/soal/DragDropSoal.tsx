...
export default function DragDropSoal({ soal, nomor, jawaban = {}, onChange }: Props) {
  ...
  const updateSlot = (slotId: string, itemId: string) => {
    const nj = { ...jawaban }
    Object.keys(nj).forEach(k => { if (nj[k] === itemId) delete nj[k] })
    nj[slotId] = itemId; onChange(soal.id, nj)
  }
  const removeFromSlot = (slotId: string) => { const nj = { ...jawaban }; delete nj[slotId]; onChange(soal.id, nj) }
  const usedItems = Object.values(jawaban)
  const available = soal.item?.filter((it: any) => !usedItems.includes(it.id)) || []
  const getItem = (id: string) => soal.item?.find((it: any) => it.id === id)
  return (
    <div className="bg-white/10 backdrop-blur border border-white/20 rounded-2xl p-4">
      ...
      <div className="bg-white/10 rounded-xl p-3 mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {available.length === 0 && <p className="text-white/40 text-xs">Semua item sudah digunakan</p>}
        </div>
      </div>
      <div className="space-y-2">
        {soal.slot?.map((slot: any) => {
          const filled = jawaban[slot.id] ? getItem(jawaban[slot.id]) : null
          return (
            <div key={slot.id} className="flex items-center gap-3">
              <p className="text-white/80 text-sm flex-1">{slot.label}</p>
              <div onDragOver={e => e.preventDefault()} onDrop={() => { if (dragItem) updateSlot(slot.id, dragItem); setDragItem(null) }}
                className={`min-w-[100px] min-h-[36px] rounded-lg border-2 border-dashed flex items-center justify-center transition-all ${filled ? 'border-green-400 bg-green-400/20' : 'border-white/30 bg-white/10 hover:border-pink-400'}`}>
                {filled ? (

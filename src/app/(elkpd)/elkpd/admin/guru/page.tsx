...
const MAPEL_COLOR: Record<string, { bg: string; border: string; text: string }> = {
      ...
      <div style={{ maxWidth:520, margin:'0 auto', padding:'24px 16px 48px', boxSizing:'border-box', width:'100%' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:20 }}>
          <button onClick={() => router.push('/elkpd/admin/dashboard')}
            style={{ background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.2)', color:'#fff', borderRadius:12, padding:'10px 14px', cursor:'pointer', fontWeight:700, fontSize:13, display:'flex', alignItems:'center', gap:6, minHeight:44 }}>
            ← Kembali
          </button>
          <div>
            <h1 style={{ color:'#fff', fontWeight:900, fontSize:20, margin:0 }}>Data Guru</h1>
            <p style={{ color:'rgba(255,255,255,0.45)', fontSize:12, margin:0 }}>{guruList.length} guru terdaftar</p>
          </div>
        </div>
        ...
        <input
          ...
          style={{ width:'100%', boxSizing:'border-box', background:'rgba(0,0,0,0.25)', border:'2px solid rgba(255,255,255,0.15)', color:'#fff', borderRadius:14, padding:'12px 16px', fontSize:16, marginBottom:16 }}
        />

        {loadingData && (
          <div style={{ textAlign:'center', padding:'40px 0', color:'rgba(255,255,255,0.5)' }}>
            <div style={{ width:32, height:32, border:'3px solid rgba(255,255,255,0.1)', borderTopColor:'#818cf8', borderRadius:'50%', margin:'0 auto 12px' }} />
            Memuat data guru...
          </div>
        )}
        ...
        {!loadingData && (
          <div style={{ background:'rgba(255,255,255,0.07)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:20, overflow:'hidden' }}>
            <div style={{ background:'rgba(99,102,241,0.2)', padding:'11px 16px', borderBottom:'1px solid rgba(255,255,255,0.1)', display:'flex', justifyContent:'space-between' }}>
              <span style={{ color:'#a5b4fc', fontSize:11, fontWeight:800, textTransform:'uppercase' }}>Nama Guru &amp; NIP</span>
              <span style={{ color:'#a5b4fc', fontSize:11, fontWeight:800, textTransform:'uppercase' }}>Mapel</span>
            </div>

            {filtered.length === 0 && (
              <div style={{ padding:'32px 16px', textAlign:'center', color:'rgba(255,255,255,0.4)', fontSize:13 }}>Tidak ada data yang cocok</div>
            )}

            {filtered.map((g, i) => {
              const c = MAPEL_COLOR[g.mapel] || defaultColor
              return (
                <div key={g.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:10, padding:'12px 16px', borderBottom: i < filtered.length - 1 ? '1px solid rgba(255,255,255,0.07)' : 'none' }}>
                  <div style={{ display:'flex', alignItems:'flex-start', gap:10, minWidth:0, flex:1 }}>
                    <span style={{ color:'rgba(255,255,255,0.3)', fontSize:12, fontWeight:700, paddingTop:3, flexShrink:0, minWidth:20 }}>{i + 1}</span>
                    <div style={{ minWidth:0 }}>
                      <p style={{ color:'#fff', fontWeight:700, fontSize:14, margin:'0 0 3px', wordBreak:'break-word' }}>{g.nama}</p>
                      <p style={{ color:'rgba(255,255,255,0.38)', fontSize:11, margin:0, fontFamily:'monospace' }}>{g.username}</p>
                    </div>
                  </div>
                  <div style={{ flexShrink:0, maxWidth:'38%' }}>
                    <span style={{ background:c.bg, border:`1px solid ${c.border}`, color:c.text, borderRadius:8, padding:'4px 8px', fontSize:11, fontWeight:700, display:'block', textAlign:'center' }}>
                      {g.mapel}
                    </span>
                  </div>
                </div>

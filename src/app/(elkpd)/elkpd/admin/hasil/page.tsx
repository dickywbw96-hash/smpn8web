'use client'
// src/app/(elkpd)/elkpd/admin/hasil/page.tsx
...
export default function HasilAdmin() {
  ...
  const [sortBy, setSortBy] = useState('submitted_at') // 'submitted_at' | 'skor_total' | 'nama'
  ...
  const fetchData = async () => {
    ...
    const [kegRes, jawRes] = await Promise.all([
      supabase
        .from('kegiatan')
        .select('id, judul, token, guru ( nama, mapel )')
        .eq('id', id)
        .single(),
      supabase
        .from('jawaban_siswa')
        .select('id, nama, tingkat, kelas, skor_otomatis, skor_uraian, skor_total, submitted_at')
        .eq('kegiatan_id', id)
        .order('submitted_at', { ascending: false }),
    ])
    setKegiatan(kegRes.data)
  ...
  return (
    <PageWrapper>
      ...
      <style>{`
        ...
        .row-item { animation: fadeUp 0.4s cubic-bezier(0.22,1,0.36,1) both; }
        ...
        {!loadingData && kegiatan && (
          <>
            ...
            <div style={{
              ...
              <p style={{ color:'rgba(255,255,255,0.45)', fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.08em', margin:'0 0 3px' }}>
                Kegiatan
              </p>
              <p style={{ color:'#fff', fontWeight:800, fontSize:15, margin:'0 0 8px' }}>
              ...
              <span style={{
                ...
                fontFamily:'monospace', fontWeight:900, fontSize:13, letterSpacing:'0.15em',
              }}>
                {kegiatan.token}
              </span>
            </div>
            ...
            <div style={{
              ...
              {[
                ...
                <div key={s.label} style={{
                  ...
                  <p style={{ color:'rgba(255,255,255,0.45)', fontSize:11, margin:0, fontWeight:600 }}>{s.label}</p>
                </div>
              ))}
            </div>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:8, marginBottom:14, alignItems:'center' }}>
              <span style={{ color:'rgba(255,255,255,0.45)', fontSize:12, fontWeight:700, flexShrink:0 }}>Urutkan:</span>
              {[
                { key:'submitted_at', label:'Terbaru' },
                ...
                <button
                  ...
                  style={{
                    background: sortBy === s.key ? 'rgba(99,102,241,0.3)' : 'rgba(255,255,255,0.07)',
                    border: `1px solid ${sortBy === s.key ? 'rgba(99,102,241,0.6)' : 'rgba(255,255,255,0.12)'}`,
                    color: sortBy === s.key ? '#a5b4fc' : 'rgba(255,255,255,0.5)',
            ...
            {sorted.length === 0 ? (
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <span style={{ color:'#4ade80', fontSize:11, fontWeight:800, textTransform:'uppercase', letterSpacing:'0.07em' }}>Nama Siswa</span>
                  <span style={{ color:'#4ade80', fontSize:11, fontWeight:800, textTransform:'uppercase', letterSpacing:'0.07em' }}>Skor</span>
                </div>
                ...
                {sorted.map((j, i) => {
                  ...
                  return (
                    ...
                    >
                      {/* Kiri: nomor + nama + kelas + waktu */}
                      <div style={{ display:'flex', gap:10, alignItems:'flex-start', minWidth:0, flex:1 }}>
                        <span style={{ color:'rgba(255,255,255,0.3)', fontSize:12, fontWeight:700, flexShrink:0, paddingTop:2, minWidth:18 }}>
                          {i + 1}
                        </span>
                        <div style={{ minWidth:0 }}>
                          <p style={{ color:'#fff', fontWeight:700, fontSize:14, margin:'0 0 2px', lineHeight:1.2, wordBreak:'break-word' }}>
                            {j.nama}
                          </p>
                          <p style={{ color:'rgba(255,255,255,0.45)', fontSize:11, margin:'0 0 2px', fontWeight:600 }}>
                            {j.tingkat} {j.kelas}
                          </p>
                          <p style={{ color:'rgba(255,255,255,0.3)', fontSize:11, margin:0 }}>
                            {j.submitted_at
                              ? new Date(j.submitted_at).toLocaleString('id-ID', { day:'2-digit', month:'short', hour:'2-digit', minute:'2-digit' })
                              : '—'}
                          </p>
                        </div>
                      </div>
                      ...
                      <div style={{ flexShrink:0, textAlign:'center' }}>
                        <span style={{
                          ...
                          color:sc.color, borderRadius:10, padding:'5px 12px',
                        ...
                        {(j.skor_uraian !== null && j.skor_uraian !== undefined) && (
                          <p style={{ color:'rgba(255,255,255,0.3)', fontSize:10, margin:'3px 0 0', textAlign:'center' }}>
                            uraian: {j.skor_uraian}
                          </p>
                        )}
                      </div>
                    </div>
